        ...
        {tab === "photo" ? (
          ...
          <>
            ...
            <section className="portrait-retouch" aria-label="피부 보정">
              <span>피부 보정</span>
              <nav>
                {([['none','off'],['on','on']] as const).map(([value,label])=><button key={value} aria-pressed={skinRetouch===value} className={skinRetouch===value ? "active" : ""} onClick={() => setSkinRetouch(value)}>{label}</button>)}
              </nav>
            </section>
          </>
        ) : tab === "frame" ? (
